...
class CartDimension(models.Model):
    ...
    @api.model
    def _merge_duplicate_dimensions(self):
        """Reuse one row per measurement without adding a database constraint."""
        grouped = {}
        dimensions = self.with_context(active_test=False).search([])
        dimensions = dimensions.sorted(
            key=lambda dimension: (not bool(dimension.is_standard), dimension.id)
        )
        for dimension in dimensions:
            key = (dimension.length, dimension.width, dimension.height)
            canonical = grouped.get(key)
            if not canonical:
                grouped[key] = dimension
                continue
            self.env['sale.order.line'].search([
                ('cart_dimension_id', '=', dimension.id),
            ]).write({'cart_dimension_id': canonical.id})
            dimension.unlink()
        return True
