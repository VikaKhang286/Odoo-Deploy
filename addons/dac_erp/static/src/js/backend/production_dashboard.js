...
// Height localStorage keys — kept for cleanup only, no longer used for restore
const _PROD_DESIGN_H = "dac_prod_design_list_h";
const _PROD_PROD_H   = "dac_prod_production_list_h";
...
class ProductionDashboard extends Component {
  setup() {
    ...
    this.productionScrollRef = useRef("productionScroll");
    ...
    onMounted(() => {
      ...
      // Xoá height cũ đã lưu — list-scroll tự co dãn theo nội dung
      localStorage.removeItem(_PROD_DESIGN_H);
      localStorage.removeItem(_PROD_PROD_H);
      this._raf1 = requestAnimationFrame(this._ensureViewportReady);
      this._refreshTimer = setInterval(() => this._silentReload(), 30_000);
    });

    onPatched(() => {
      // Không restore height từ localStorage — chiều cao theo nội dung
    });
  ...
  startResizeDesign(ev) {
    this._startDrag(ev, this.designScrollRef.el, _PROD_DESIGN_H);
  }

  startResizeProduction(ev) {
    this._startDrag(ev, this.productionScrollRef.el, _PROD_PROD_H);
  }
  ...
  async _silentReload() {
    try {
      const data = await this.orm.call("sale.order", "dac_get_dashboard_production", []);
      if (data) this.state.data = data;
    } catch (e) {
      console.error("[production_dashboard] silent reload failed:", e);
    }
  }
  ...
  get productionTasks() {
    return this.state.data?.production_tasks || [];
  }
  get overdueDesignCount() {
    return this.designTasks.filter((t) => t.is_overdue).length;
  }
  get overdueProductionCount() {
    return this.productionTasks.filter((t) => t.is_overdue).length;
  }
  ...
  get filteredProductionTasks() {
    return this._applyFilter(this.productionTasks);
  }
...
registry.category("actions").add("dac_production_dashboard", ProductionDashboard);
